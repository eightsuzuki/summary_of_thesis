...
**LIMEの基本的なアイデア**:
- **局所近似**: ブラックボックスモデル全体を説明するのではなく、**特定の入力 \(x\) の近傍**でモデルを線形モデルで近似する
- **解釈可能性**: 近似モデル \(g\) は人間が理解できる形式（例: スパース線形モデル、決定木）である
- **モデル非依存**: 任意のブラックボックス分類器に適用可能（勾配や内部構造に依存しない）

**直感的な説明**:
複雑な関数 \(f(x)\) を、点 \(x\) の近くだけに注目して「この近くでは \(f(x) \approx w_1 x_1 + w_2 x_2 + \cdots + b\) のように線形で近似できる」と考える。この線形近似の係数 \(w_i\) が「特徴 \(i\) が予測にどれだけ寄与したか」を表す説明となる。
...
**適用可能なデータタイプ**:
- **テキストデータ**: 単語の有無を特徴として扱う。例: 感情分析、スパム検出
  - 解釈可能表現: 各単語が存在するか（1）しないか（0）
  - サンプリング: 元のテキストから一部の単語をランダムに削除したバリエーションを生成
- **画像データ**: スーパーピクセル（類似色のピクセル群）を特徴として扱う。例: 画像分類
  - 解釈可能表現: 各スーパーピクセルがON（1）かOFF（0）か
  - サンプリング: 一部のスーパーピクセルをOFF（平均色で塗り潰す）したバリエーションを生成
- **表形式データ**: 各特徴量をそのまま使用。例: 金融リスク評価、医療診断

**使用例**:
- 画像分類: 「なぜこの画像が"猫"と分類されたか」→ 重要なスーパーピクセル（猫の顔、耳など）を可視化
- テキスト分類: 「なぜこのレビューが"ポジティブ"と分類されたか」→ 重要な単語（"素晴らしい"、"最高"など）を強調
- 金融: 「なぜこの申請が"拒否"されたか」→ 重要な特徴（収入、信用スコアなど）を提示
...
**LIME vs Anchors**:
- **LIME**: 局所的な線形近似により、特徴の寄与度（重み）を連続値で提供
- **Anchors**: 高精度保証つきのIf-Thenルール（離散的な条件）を提供
- **補完的**: LIMEは「どの特徴がどれだけ重要か」、Anchorsは「この条件が満たされれば予測は変わらない」を説明

**LIME vs Grad-CAM**:
- **LIME**: モデル非依存、任意の分類器に適用可能、勾配不要
- **Grad-CAM**: CNN専用、勾配情報を使用、アーキテクチャ変更不要
- **適用範囲**: LIMEはより汎用的だが、Grad-CAMはCNNに対してより効率的

**LIME vs Integrated Gradients**:
- **LIME**: サンプリングベース、局所近似、モデル非依存
- **Integrated Gradients**: 勾配ベース、経路積分、微分可能モデルに適用
- **計算コスト**: LIMEは多数のサンプルが必要、Integrated Gradientsは1回の逆伝播で計算可能
