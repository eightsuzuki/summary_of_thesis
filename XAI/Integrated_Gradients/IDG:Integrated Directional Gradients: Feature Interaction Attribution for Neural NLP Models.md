...
2.  **$z^S$ベクトルの役割**:
    この $z^S$ は、**「方向微分（Directional Derivative）」を計算するための「射影ベクトル」**として使われます。モデルの勾配 $\nabla f$（全次元に対する感度）に対して、この $z^S$ との内積（dot product）を取ります。すると、**マスクされた部分（グループ $S$ 以外）の勾配はすべて $0$ を掛けられて消去されます**。これにより、グループ $S$ の方向へのみの変化率（方向微分）が計算されます。
