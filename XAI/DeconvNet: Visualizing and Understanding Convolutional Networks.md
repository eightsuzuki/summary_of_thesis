...
#### 2.1.1 DeconvNetの計算過程：直感的な説明

**DeconvNetは結局何をしているか**:
中間層の活性化 \(a_{\ell}\)（「このフィルタがどこで反応したか」）を、入力画像空間に「逆投影」して、元の画像のどの部分がその活性化を引き起こしたかを可視化する。

**具体的な計算ステップ**:

**ステップ1: Unpooling（逆プーリング）**
- **問題**: Max-poolingで空間サイズが縮小し、情報が失われている
- **解決**: 順伝播時に「どの位置が最大値だったか」（switch）を記録しておき、逆方向ではその位置にのみ値を戻す
- **例**: 2×2のMax-poolingで、左上が最大値だった場合、逆方向では左上にのみ値を配置し、他は0

**ステップ2: Rectification（整流）**
- **操作**: 負の値を0にクリップ（\(\max(0, \cdot)\)）
- **理由**: 可視化をスパースにし、直感的なパターンを強調するため
- **効果**: ノイズを減らし、重要な部分だけを強調

**ステップ3: Deconvolution（転置畳み込み）**
- **操作**: 畳み込みフィルタ \(W_{\ell}\) を転置・空間反転した \(W_{\ell}^{\top}\) で畳み込み
- **意味**: 畳み込みの「逆操作」を近似的に実行
- **注意**: 完全な逆変換ではないが、構造的に対応する操作として機能

**全体の流れ**:
```
中間層の活性化 a_ℓ
    ↓
[Unpooling] スイッチ位置に値を戻す
    ↓
[Rectification] 負の値を0にクリップ
    ↓
[Deconvolution] 転置畳み込みで前の層へ
    ↓
... (これを繰り返す) ...
    ↓
入力画像空間 r_0（可視化結果）
...
**重要なポイント**:
- **順伝播時の情報保存**: Max-poolingのswitch位置を保存しておく必要がある（これがないと逆投影できない）
- **近似操作**: 完全な逆変換ではなく、構造的に対応する操作
- **層ごとに繰り返す**: 各層で上記3ステップを実行し、最終的に入力画像空間まで戻す

**数式で見る全体の計算**:
層 \(\ell\) から層 \(\ell-1\) への逆投影は：
\[
r_{\ell-1} = W_{\ell}^{\top} * \max(0, U_{S_{\ell}}(r_{\ell}))
\]
これを各層で繰り返し、最終的に：
\[
r_0 = \text{DeconvNet}(a_{\ell})
\]
として、入力画像空間での可視化結果を得る。
...
### 3.5. その後の進化：Saliency Mapsへの発展

**DeconvNetの限界**:
- 複雑な逆投影操作（Unpooling → Rectification → Deconvolution）が必要
- 中間層の活性化を可視化するが、予測の説明には間接的
- 実装が複雑で、計算コストが高い

**Saliency Maps（Simonyan et al., 2013）による進化**:
- **勾配ベースの統一的枠組み**: 単純に勾配 \(\nabla_x S_c(x)\) を計算するだけで、クラス特異的なサリエンシマップを生成
- **計算の簡素化**: 複雑な逆投影操作が不要で、1回の逆伝播で計算可能
- **目的の明確化**: 中間層の理解から、予測の説明へ直接的にアプローチ
- **理論的統一**: 標準BP、DeconvNet、Guided-BPを統一的に理解

**主な進化のポイント**:
1. **方法**: 逆投影 → 勾配計算（より直接的）
2. **目的**: 中間層の可視化 → 予測の説明（より実用的）
3. **計算**: 複雑な操作 → 単純な勾配（より高速）
4. **応用**: 可視化・理解 → 弱教師あり局在など（より広範）

詳細は [Saliency Maps: Deep Inside Convolutional Networks](./Saliency%20Maps:%20Deep%20Inside%20Convolutional%20Networks:%20Visualising%20Image%20Classification%20Models%20and%20Saliency%20Maps.md) を参照。
